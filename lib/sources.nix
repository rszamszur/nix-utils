...
let
  ...
  fileHasAnySuffix = fileSuffixes: file: (any (s: hasSuffix s file.name) fileSuffixes);

  mkUnions =
    rootPath: subPaths: unions (map (subPath: maybeMissing (rootPath + "/${subPath}")) subPaths);

  toFilesets =
    rootPath: ignoredPaths:
    mapAttrsRecursiveCond (as: !(isList as)) (_: v: mkUnions rootPath v) ignoredPaths;
  ...
  languageFileFilters =
    let
      toFilesets =
        ...
        in
        mapAttrsRecursiveCond (as: !(isFunction as)) (_: v: mkFileFilter v) ignoreFilters;
    in
    rootPath:
    toFilesets rootPath {
      ...
      python = {
        ...
        pyTestCache = (file: file.name == ".pytest_cache" || hasPrefix ".coverage." file.name);
        eggInfo = (file: hasSuffix ".egg-info" file.name);
      };
      secrets = (
        file: hasPrefix ".decrypted~" file.name || fileHasAnySuffix [ ".agekey" ".pub" ".key" ".pem" ] file
      );
    };
  filterSources =
    {
